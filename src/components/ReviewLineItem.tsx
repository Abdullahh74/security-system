  ...
  return (
    <div className="flex items-center gap-2.5 py-1">
      <ProductImage
        src={line.image}
        alt={line.title}
        className={`h-10 w-10 shrink-0 rounded-md ${isCamUnlimited ? "border-transparent bg-info-bg" : "border border-border-light bg-white"}`}
      />
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium text-heading-text">
          {line.title === "Cam Unlimited" ? (
            <>
              <span className="font-semibold">Cam</span> <span className="font-semibold text-primary">Unlimited</span>
            </>
          ) : (
            line.title
          )}
